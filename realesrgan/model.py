...
from realesrgan.fp16 import FP16Module
...
class RealESRGAN:
    ...
    def predict(self, lr_image, batch_size=4, patches_size=192,
        ...
        sr_image = res.permute((0, 2, 3, 1)).clamp_(0, 1)
        # np_sr_image = sr_image.numpy()

        padded_size_scaled = tuple(np.multiply(p_shape[0:2], scale)) + (3,)
        scaled_image_shape = tuple(np.multiply(lr_image.shape[0:2], scale)) + (3,)
        del img, res, lr_image, patches
        sr_image = stich_together(sr_image, padded_image_shape=padded_size_scaled,
                                     target_shape=scaled_image_shape, padding_size=padding * scale, device=device)
        sr_image = (sr_image.cpu().numpy() * 255).astype(np.uint8)
